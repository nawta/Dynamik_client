...
using System.Collections;
#endif
public class TestAudiov3 : MonoBehaviour
{
    ...
    private void RecognizingHandler(object sender, TranslationRecognitionEventArgs e)
    {
        lock (threadLocker)
        {
            ...
            if (hasIntervalPassed)
            {
                ...
                {
                    message = InsertNewlines(QueryText2API(element.Value)); //ここは話している最中の未完成の文章に対応する
    ...
    private void RecognizedHandler(object sender, TranslationRecognitionEventArgs e)
    {
        lock (threadLocker)
        {
            ...
            foreach (var element in e.Result.Translations)
            {
                message = InsertNewlines(QueryText2API(element.Value)); //ここは話し終わったあとの完成形の文章に対応する. ここにQueryText2APIを挟むと要約タスクが挟めたりする
